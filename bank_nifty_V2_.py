...
# Method 2: More memory efficient - seek from end of file
def read_last_line_efficient(filename):
    with open(filename, 'rb') as file:
        ...
        # Read backwards until we find a newline (start of last line)
        while file.tell() > 0:
            file.seek(-1, 1)
            char = file.read(1)
            if char == b'\n':
                break
            file.seek(-1, 1)  # Move back again since read() advances position

        # Read the last line
        last_line = file.readline().decode('utf-8').strip()
        return last_line
# Function to get the 5th field from a line (using : as separator)
def get_seventh_field(line, separator=':'):
    ...
    # Check if we have at least 8 fields (index 7 for 8th field)
    if len(fields) >= 5:
        try:
            #convert to float immediately
            return float(fields[6])  # 8th field is at index 6
        except (ValueError, TypeError):
            return 0.0
    else:
        return 0.0

def get_twelth_field(line, separator=':'):
    if not line:
        return None
    fields = line.split(separator)
    if len(fields) >= 9:
        try:
            return float(fields[10])  # 12th field is index 11
        except (ValueError, TypeError):
            return 0.0
    else:
        return 0.0
...
if __name__ == '__main__':
    ...
    now = datetime.now(tz=ZoneInfo('Asia/Kolkata'))
    print("now =", now)
    lfile = now.strftime("./logs/abcd_0945_bn_straddle.txt")
    ...
    #fin = open('./logs/%Y%m%d_rt_0945_bn_straddle.txt','r')
    #filename ='./logs/%Y%m%d_rt_0945_bn_straddle.txt'
    filename = './logs/abcd_0945_bn_straddle.txt'
...
# declaration for the plot
    ...
    while (True):
        ...
        print((f'{day}:{hr}:{min}::{symbol_ce}:{ltpce}:{getCMP(symbol_cep)}:{plce}:{slcehit} - {symbol_pe}:{ltppe}:{getCMP(symbol_pep)}:{plpe}:{slpehit}:-- > {pl}::{plmax}::{plmin}'))
                ...
                #--------------------- code here to plot live -----------------------
        # Read the latest line or end line of a file pointed by fin
        print("Efficient method:", read_last_line_efficient(filename))
        rline = read_last_line_efficient(filename)
        separator=':'
        ltpce_new = get_seventh_field(rline, separator)
        ltppe_new = get_twelth_field(rline, separator)
        print(ltpce_new)
        print(ltppe_new)

        # Convert to float safely
        try:
            ltpce_new_float = float(ltpce_new) if ltpce_new is not None else 0.0
        except (ValueError, TypeError):
            ltpce_new_float = 0.0
            print(f"Warning: Could not convert '{ltpce_new}' to float, using 0.0")
        
        try:
            ltppe_new_float = float(ltppe_new) if ltppe_new is not None else 0.0
        except (ValueError, TypeError):
            ltppe_new_float = 0.0
            print(f"Warning: Could not convert '{ltppe_new}' to float, using 0.0")
        ...
        ltpce_val.append(ltpce_new_float)
        ltppe_val.append(ltppe_new_float)

        # Fixed plotting code
        try:
            if 'line1' not in locals():
                fig, ax = plt.subplots(figsize=(14, 6))
                line1, = ax.plot(time_val, ltpce_val, 'b-', linewidth=2, marker='o', markersize=3, label='CE Price')
                line2, = ax.plot(time_val, ltppe_val, 'r-', linewidth=2, marker='s', markersize=3, label='PE Price')
                ax.set_xlabel('Time', fontsize=12)
                ax.set_ylabel('P&L Value', fontsize=12)
                ax.set_title('Real-time CE and PE P&L', fontsize=14)
                ...
                line1.set_ydata(ltpce_val)
                line2.set_xdata(time_val)
                line2.set_ydata(ltppe_val)
