...
if __name__ == '__main__':
    ...
    if offline == 0:
        lfile = now.strftime("./logs/%Y%m%d_%H%M_dual_crossover_nifty50_straddle_online.txt")
    else:
        lfile = now.strftime("./logs/%Y%m%d_%H%M_dual_crossover_nifty50_straddle_offline.txt")
    ...
    filename = lfile
    file1 = open("./logs/dual_straddle_945am", "a")
    papertrade = 0
    ...
    if offline == 1:
        INPUT_FILE = "edata_c_pe.xlsx"
        OUTPUT_FILE = f"logs/dual_stg_{INPUT_FILE}"
    ...
    try:
        while True:
            ...
            if offline == 1:
                if i >= len(df):
                    ...
                    chart_filename = now.strftime("./logs/%Y%m%d_%H%M%S_dual_crossover_nifty50_straddle_offline.png")
            ...
            if(datetime.now(tz=ZoneInfo('Asia/Kolkata')).hour == 15) and (datetime.now(tz=ZoneInfo('Asia/Kolkata')).minute == 30):
                ...
                now = datetime.now(tz=ZoneInfo('Asia/Kolkata'))
                if offline == 0:
                    chart_filename = now.strftime("./logs/%Y%m%d_%H%M%S_dual_crossover_nifty50_straddle_online.png")
                else:
                    chart_filename = now.strftime("./logs/%Y%m%d_%H%M%S_dual_crossover_nifty50_straddle_offline.png")
                fig.savefig(chart_filename, dpi=300)
                print(f"Chart saved as: {chart_filename}")
                ...
                print(pl)
                try:
                    p_apl = subprocess.check_output("cat ./logs/dual_straddle_945am|awk 'END{print}'|awk -F \":\" '{print $19;}'",shell=True)
        ...
        print(f"- Summary log: ./logs/dual_straddle_945am")
        if offline == 1:
            print(f"- Excel output: {OUTPUT_FILE}")
        print("="*80)
    finally:
        if 'fig' in locals() and fig is not None and not chart_saved:
            chart_filename = datetime.now(tz=ZoneInfo('Asia/Kolkata')).strftime("./logs/%Y%m%d_%H%M%S_dual_crossover_nifty50_straddle_offline.png")
            fig.savefig(chart_filename, dpi=300)
            print(f"Chart saved as: {chart_filename}")
