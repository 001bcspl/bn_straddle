...
qsma20 = Queue(maxsize=20)

def calculate_sma(q):
    items = list(q.queue)  # Convert Queue to list
    if not items:
        return None
    return sum(items) / len(items)


def update_q(q, value):
    if q.full():
        q.get()         # Remove oldest
    q.put(value)        # Add new value
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        # === Append CE price and calculate SMA20 ===
        ce_prices.append(ltpce1)
        now_time = datetime.now(tz=ZoneInfo('Asia/Kolkata')).strftime('%H:%M:%S')
        time_labels.append(now_time)
        update_q(qsma20, ltpce1)
        sma20 = calculate_sma(qsma20)
        
        
        if qsma20.qsize() == 20:
            items = list(qsma20.queue)
            sma20 = round(sum(items) / 20, 2)
            sma20_vals.append(sma20)
        else:
            sma20_vals.append(None)
        
        # === Plotting the SMA20 chart for CE live ===
        plt.clf()
        plt.title(f'CE Price and SMA20 Live - {symbol_ce}')
        plt.plot(time_labels, ce_prices, label='CE Price', color='blue')
        plt.plot(time_labels, sma20_vals, label='SMA20_CE', color='orange')
        plt.xticks(rotation=45)
        plt.xlabel('Time')
        plt.ylabel('CE_Price & SMA20_CE_Price')
        plt.legend()
        plt.tight_layout()
        plt.grid(True)
        plt.pause(0.1)  # Add this line to allow the interactive plot to update
